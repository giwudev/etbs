		...
		<tbody>
			@foreach($list as $listgiwu)
				<tr>
					...
					<td>
						@if($listgiwu->statut_trimSem == 'a')<span class="badge bg-success">{{trans('entite.statutanne')[$listgiwu->statut_trimSem]}}</span> @else <span class="badge bg-danger">{{trans('entite.statutanne')[$listgiwu->statut_trimSem]}}</span> @endif
					</td>
					...
					@if(in_array('update_trimsem',session('InfosAction')) || in_array('delete_trimsem',session('InfosAction')) )
						<td class="text-center">
							@if(in_array('update_trimsem',session('InfosAction')))
								<a href="{{route('trimsem.edit',$listgiwu->id_trimSem)}}" title='Modifier' class="btn btn-success btn-sm  waves-effect waves-light"><i class="ri-edit-2-line"></i></a>
							@endif
							@if(in_array('delete_trimsem',session('InfosAction')))
								<button type="button"  title='Supprimer' data-id="{{$listgiwu->id_trimSem}}" class="btn btn-danger btn-sm  waves-effect waves-light btn-delete" data-bs-toggle="modal" ><i class="ri-delete-bin-6-line"></i></button>
							@endif
					@endif
				</tr>
			@endforeach
