...
@section('content')
	...
	<div class="col-lg-12">
		<div class="card"  id="ticketsList">
				...
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre}}</h5>
					<div class="flex-shrink-0">
						@if(in_array('add_trimsem',session('InfosAction')))
							<a class="btn btn-primary btn-label right" href="{{route('trimsem.create')}}"><i class="ri-add-line label-icon align-middle fs-16 ms-2"></i>Ajouter</a>
						@endif
						@if(in_array('exporter_trimsem',session('InfosAction')))
					...
					<div class="row gy-4">
						...
						<div class="col-xxl-3 col-md-4">
							<div><label for="labelInput" class="form-label">Liste des Années</label>
								<?php $addUse = array(''=>'Sélectionnez un élément'); $listannee_id = $addUse + $listannee_id->toArray();?>
			...
			<div class="card-body"><div id='dataRefresh' class="table-responsive table-card mb-4 m-2 giwuRefresh">@include('trimsem.index-search')</div></div>
		</div><!--end card-->
	</div><!--end col-->
...
@section('JS_content')
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".exporterXls").attr('href','{{url("trimsem/exporterExcel")}}');
			$(".exporterPdf").attr('href','{{url("trimsem/exporterPdf")}}');
			// Pagination
			...
			function fetch_page(page){
				...
				$.ajax({ url:"trimsem?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			$(".exporterXls").attr('href','{{url("trimsem/exporterExcel")}}?'+filtreData);
			$(".exporterPdf").attr('href','{{url("trimsem/exporterPdf")}}?'+filtreData);
			...
			return $.ajax({
				url: '{{ url("/trimsem/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.btn-delete', function () {
			id = $(this).data("id");
			$.ajax({url : '{{ url("trimsem/AffichePopDelete/") }}/'+id,type : 'GET',dataType : 'html',
