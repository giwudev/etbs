...
@section('content')
	...
	<div class="col-lg-12">
		<div class="card"  id="ticketsList">
			<div class="card-header border-0">
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre}}</h5>
					<div class="flex-shrink-0">
						@if(in_array('add_promotion',session('InfosAction')))
							<a class="btn btn-primary btn-label right" href="{{route('promotion.create')}}"><i class="ri-add-line label-icon align-middle fs-16 ms-2"></i>Ajouter</a>
						@endif
						@if(in_array('exporter_promotion',session('InfosAction')))
				...
				{!! Form::open(array('id' => 'formSearch','class' => '', 'method' => 'GET')) !!}
					<div class="row gy-4">
						...
						<div class="col-xxl-3 col-md-4">
							<div><label for="labelInput" class="form-label">Liste des Classes</label>
								<?php $addUse = array(''=>'Selectionnez un element'); $listclass_id = $addUse + $listclass_id->toArray();?>
								{!! Form::select('class_id',$listclass_id ,session('class_idSess'),["id"=>"class_id","onchange"=>"funcRecher()","class"=>"form-select allselect"]) !!}
							</div>
						</div>
			...
			<div class="card-body"><div id='dataRefresh' class="table-responsive table-card mb-4 m-2 giwuRefresh">@include('promotion.index-search')</div></div>
		</div><!--end card-->
	</div><!--end col-->
...
@section('JS_content')
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".exporterXls").attr('href','{{url("promotion/exporterExcel")}}');
			$(".exporterPdf").attr('href','{{url("promotion/exporterPdf")}}');
			// Pagination
			...
			function fetch_page(page){
				...
				$.ajax({ url:"promotion?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			$(".exporterXls").attr('href','{{url("promotion/exporterExcel")}}?'+filtreData);
			$(".exporterPdf").attr('href','{{url("promotion/exporterPdf")}}?'+filtreData);
			...
			return $.ajax({
				url: '{{ url("/promotion/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.btn-delete', function () {
			id = $(this).data("id");
			$.ajax({url : '{{ url("promotion/AffichePopDelete/") }}/'+id,type : 'GET',dataType : 'html',
