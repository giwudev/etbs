		...
		<div class="card">
			...
			<div class="card-body"><p class="text-muted"></p>
				<div class="live-preview"><strong><div class="msgAjouter"></div></strong>
					<form action="{{route('promotion.store')}}" method="post" id="form" class="row g-3 needs-validation" novalidate >
						@csrf()
							<div class="row">
							...
							<div class="col-md-6">
								<div class="mb-3">
									<label for="libelle_pro" class="form-label">{!!trans('data.libelle_pro')!!} <strong style='color: red;'> *</strong></label>
									{!! Form::text('libelle_pro','',["id"=>"libelle_pro","class"=>"form-control" ,"required"=>"required" ,'autocomplete'=>'off' ,'placeholder'=>"Entrer Promotion" ]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									<label for="class_id" class="form-label">{!!trans('data.class_id')!!} <strong style='color: red;'> *</strong></label>
									<?php $addUse = array(''=>'S&eacute;lectionnez un &eacute;l&eacute;ment'); $listclass_id = $addUse + $listclass_id->toArray();?>
									{!! Form::select('class_id',$listclass_id ,session('class_idSess'),["id"=>"class_id","class"=>"form-select allselect" ,"required"=>"required"]) !!}
								</div>
							</div>
							<div class="col-12">
								<div class="text-end">
									<a href="{{route('promotion.index')}}" class="btn btn-outline-dark waves-effect mr-10">Fermer</a>
