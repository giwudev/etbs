...
@if(count($list) != 0)
	<table class="table table-striped table-bordered table-nowrap">
		<thead><tr>
			<th scope="col" >{!!trans('data.heure_debut')!!}</th>
		...
		<tbody>
			@foreach($list as $listgiwu)
				<tr>
					...
					<td style ='text-align:right' >{{strrev(wordwrap(strrev(intval($listgiwu->jour_semaine)), 3, ' ', true))}}</td>
					...
					@if(in_array('update_emploitemp',session('InfosAction')) || in_array('delete_emploitemp',session('InfosAction')) )
						<td class="text-center">
							@if(in_array('update_emploitemp',session('InfosAction')))
								<a href="{{route('emploitemp.edit',$listgiwu->id_empl)}}" title='Modifier' class="btn btn-success btn-sm  waves-effect waves-light"><i class="ri-edit-2-line"></i></a>
							@endif
							@if(in_array('delete_emploitemp',session('InfosAction')))
								<button type="button"  title='Supprimer' data-id="{{$listgiwu->id_empl}}" class="btn btn-danger btn-sm  waves-effect waves-light btn-delete" data-bs-toggle="modal" ><i class="ri-delete-bin-6-line"></i></button>
							@endif
					@endif
				</tr>
			@endforeach
