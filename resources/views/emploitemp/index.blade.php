				...
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre}}</h5>
					<div class="flex-shrink-0">
						@if(in_array('add_emploitemp',session('InfosAction')))
							<a class="btn btn-primary btn-label right" href="{{route('emploitemp.create')}}"><i class="ri-add-line label-icon align-middle fs-16 ms-2"></i>Ajouter</a>
						@endif
			...
			<div class="card-body border border-dashed border-end-0 border-start-0">
				{!! Form::open(array('id' => 'formSearch','class' => '', 'method' => 'GET')) !!}
					<div class="row gy-4">
						...
						<!--end col-->
						<div class="col-xxl-3 col-md-3"> <!--Code en commentaire a remplacer display:none si l'element est parametre -->
							<?php // echo ((!in_array('combo_emploitemp,session('InfosAction'))) ? "style='display:none;'" : '')?>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			function fetch_page(page){
				...
				$.ajax({ url:"emploitemp?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			$(".exporterXls").attr('href','{{url("emploitemp/exporterExcel")}}?'+filtreData);
			$(".exporterPdf").attr('href','{{url("emploitemp/exporterPdf")}}?'+filtreData);
			...
			return $.ajax({
				url: '{{ url("/emploitemp/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.btn-delete', function () {
			id = $(this).data("id");
			$.ajax({url : '{{ url("emploitemp/AffichePopDelete/") }}/'+id,type : 'GET',dataType : 'html',
