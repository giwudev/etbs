					...
					@if(in_array('update_frequenter',session('InfosAction')) || in_array('delete_frequenter',session('InfosAction')) )
						<td class="text-center">
							@if(in_array('update_frequenter',session('InfosAction')))
								<a href="{{route('frequenter.edit',$listgiwu->id_freq)}}" title='Modifier' class="btn btn-success btn-sm  waves-effect waves-light"><i class="ri-edit-2-line"></i></a>
							@endif
							@if(in_array('delete_frequenter',session('InfosAction')))
								<button type="button"  title='Supprimer' data-id="{{$listgiwu->id_freq}}" class="btn btn-danger btn-sm  waves-effect waves-light btn-delete" data-bs-toggle="modal" ><i class="ri-delete-bin-6-line"></i></button>
							@endif
					@endif
				</tr>
			@endforeach
