		...
		<div class="card"  id="ticketsList">
				...
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre}}</h5>
					<div class="flex-shrink-0">
						@if(in_array('add_frequenter',session('InfosAction')))
							<a class="btn btn-primary btn-label right" href="{{route('frequenter.create')}}"><i class="ri-add-line label-icon align-middle fs-16 ms-2"></i>Ajouter</a>
						@endif
			...
			<div class="card-body border border-dashed border-end-0 border-start-0">
				{!! Form::open(array('id' => 'formSearch','class' => '', 'method' => 'GET')) !!}
					<div class="row gy-4">
						<!--end col-->
						<div class="col-xxl-3 col-md-4" style='display:none;'> <!--Code en commentaire a remplacer display:none si l'element est parametre -->
							...
							<div><label for="labelInput" class="form-label">Liste des élèves</label>
								<?php $addUse = array(''=>'Selectionnez un element'); $listeleve_id = $addUse + $listeleve_id->toArray();?>
								{!! Form::select('eleve_id',$listeleve_id ,session('eleve_idSess'),["id"=>"eleve_id","onchange"=>"funcRecher()","class"=>"form-select allselect"]) !!}
							</div>
						</div>
			...
			<div class="card-body"><div id='dataRefresh' class="table-responsive table-card mb-4 m-2 giwuRefresh">@include('frequenter.index-search')</div></div>
		</div><!--end card-->
	</div><!--end col-->
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			function fetch_page(page){
				...
				$.ajax({ url:"frequenter?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			return $.ajax({
				url: '{{ url("/frequenter/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.btn-delete', function () {
			id = $(this).data("id");
			$.ajax({url : '{{ url("frequenter/AffichePopDelete/") }}/'+id,type : 'GET',dataType : 'html',
