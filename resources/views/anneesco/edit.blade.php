		...
		<div class="card">
			...
			<div class="card-body"><p class="text-muted"></p>
				<div class="live-preview"><strong><div class="msgAjouter"></div></strong>
					<form action="{{route('anneesco.update',$item->id_annee)}}" method="post" id="form" class="row g-3 needs-validation" novalidate >
						@csrf()
						@method('PATCH')
							<div class="row">
							...
							<div class="col-md-6">
								<div class="mb-3">
									...
									{!! Form::number('annee_debut',$item->annee_debut,["id"=>"annee_debut","class"=>"form-control" ,"required"=>"required" ,'autocomplete'=>'off' ,'placeholder'=>"Entrer Annee debut" ]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									<label for="annee_fin" class="form-label">{!!trans('data.annee_fin')!!} <strong style='color: red;'> *</strong></label>
									{!! Form::number('annee_fin',$item->annee_fin,["id"=>"annee_fin","class"=>"form-control" ,"required"=>"required" ,'autocomplete'=>'off' ,'placeholder'=>"Entrer Annee fin" ]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									<label for="statut_annee" class="form-label">{!!trans('data.statut_annee')!!} <strong style='color: red;'> *</strong></label>
									{!! Form::select('statut_annee',trans('entite.statutanne'),$item->statut_annee,["id"=>"statut_annee","class"=>"form-select allselect" ,"required"=>"required"]) !!}
								</div>
							</div>
							<div class="col-md-6">
								<div class="mb-3">
									...
									{!! Form::select('etablis_id',$listetablis_id ,$item->etablis_id,["id"=>"etablis_id","class"=>"form-select allselect" ,"required"=>"required"]) !!}
								</div>
							</div>
							<div class="col-12">
								<div class="text-end">
