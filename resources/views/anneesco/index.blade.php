...
@section('content')
	...
	<div class="col-lg-12">
		<div class="card"  id="ticketsList">
				...
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre}}</h5>
					<div class="flex-shrink-0">
						@if(in_array('add_anneesco',session('InfosAction')))
							<a class="btn btn-primary btn-label right" href="{{route('anneesco.create')}}"><i class="ri-add-line label-icon align-middle fs-16 ms-2"></i>Ajouter</a>
						@endif
						@if(in_array('exporter_anneesco',session('InfosAction')))
			...
			<div class="card-body"><div id='dataRefresh' class="table-responsive table-card mb-4 m-2 giwuRefresh">@include('anneesco.index-search')</div></div>
		</div><!--end card-->
	</div><!--end col-->
...
@section('JS_content')
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".exporterXls").attr('href','{{url("anneesco/exporterExcel")}}');
			$(".exporterPdf").attr('href','{{url("anneesco/exporterPdf")}}');
			// Pagination
			...
			function fetch_page(page){
				...
				$.ajax({ url:"anneesco?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			$(".exporterXls").attr('href','{{url("anneesco/exporterExcel")}}?'+filtreData);
			$(".exporterPdf").attr('href','{{url("anneesco/exporterPdf")}}?'+filtreData);
			...
			return $.ajax({
				url: '{{ url("/anneesco/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.btn-delete', function () {
			id = $(this).data("id");
			$.ajax({url : '{{ url("anneesco/AffichePopDelete/") }}/'+id,type : 'GET',dataType : 'html',
