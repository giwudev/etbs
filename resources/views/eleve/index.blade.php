...
@section('content')
	...
	<div class="col-lg-12">
		<div class="card"  id="ticketsList">
				...
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre}}</h5>
					<div class="flex-shrink-0">
						@if(in_array('add_eleve',session('InfosAction')))
							<a class="btn btn-primary btn-label right" href="{{route('eleve.create')}}"><i class="ri-add-line label-icon align-middle fs-16 ms-2"></i>Ajouter</a>
						@endif
						@if(in_array('exporter_eleve',session('InfosAction')))
				...
				{!! Form::open(array('id' => 'formSearch','class' => '', 'method' => 'GET')) !!}
					<div class="row gy-4">
						...
						<div class="col-xxl-3 col-md-4"> <!--Code en commentaire a remplacer display:none si l'element est parametre -->
							<?php // echo ((!in_array('combo_eleve,session('InfosAction'))) ? "style='display:none;'" : '')?>
			...
			<div class="card-body"><div id='dataRefresh' class="table-responsive table-card mb-4 m-2 giwuRefresh">@include('eleve.index-search')</div></div>
		</div><!--end card-->
	</div><!--end col-->
...
@section('JS_content')
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".exporterXls").attr('href','{{url("eleve/exporterExcel")}}');
			$(".exporterPdf").attr('href','{{url("eleve/exporterPdf")}}');
			// Pagination
			...
			function fetch_page(page){
				...
				$.ajax({ url:"eleve?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			$(".exporterXls").attr('href','{{url("eleve/exporterExcel")}}?'+filtreData);
			$(".exporterPdf").attr('href','{{url("eleve/exporterPdf")}}?'+filtreData);
			...
			return $.ajax({
				url: '{{ url("/eleve/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.btn-delete', function () {
			id = $(this).data("id");
			$.ajax({url : '{{ url("eleve/AffichePopDelete/") }}/'+id,type : 'GET',dataType : 'html',
