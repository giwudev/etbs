		...
		<div class="card">
			...
			<div class="card-body"><p class="text-muted"></p>
				<div class="live-preview"><strong><div class="msgAjouter"></div></strong>
					<form action="{{route('paiementprof.store')}}" method="post" id="form" class="row g-3 needs-validation" novalidate >
						@csrf()
							<div class="row">
							@if ($errors->any())
							...
							<div class="col-md-6">
								<div class="mb-3">
									...
									{!! Form::select('id_prof',$listid_prof ,session('id_profSess'),["id"=>"id_prof","class"=>"form-select allselect" ,"required"=>"required"]) !!}
								</div>
							</div>
							<div class="col-md-3">
								<div class="mb-3">
									<label for="datedebut" class="form-label">{!!trans('data.datedebut')!!} <strong style='color: red;'> *</strong></label>
									{!! Form::date('datedebut',old('datedebut'),["id"=>"datedebut","class"=>"form-control" ,"required"=>"required" ,'autocomplete'=>'off' ,'placeholder'=>"Entrer Date debut" ]) !!}
								</div>
							</div>
							<div class="col-md-3">
								<div class="mb-3">
									<label for="datefin" class="form-label">{!!trans('data.datefin')!!} <strong style='color: red;'> *</strong></label>
									{!! Form::date('datefin',old('datefin'),["id"=>"datefin","class"=>"form-control" ,"required"=>"required" ,'autocomplete'=>'off' ,'placeholder'=>"Entrer Date fin" ]) !!}
								</div>
							</div>
						</div>
						<div class="col-md-12 mt-2"><div class="alert alert-success alert-border-left alert-dismissible fade show" role="alert"><i title ="" class="ri-notification-off-line me-3 align-middle"></i> <strong>Détails du paiement</strong></div></div>
						<br>
						<!--end row-->
						<div class="row">
							<div class="col-12">
								<table class="table table-bordered" id="paiementTable">
									...
									<tbody>
										...
										@foreach(old('description', []) as $index => $description)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td><input type="text" name="description[]" class="form-control" value="{{ $description }}" required></td>
                                                <td><input type="number" name="qte[]" class="form-control" value="{{ old('qte.' . $index) }}" required oninput="calculateAmount(this)"></td>
                                                <td><input type="number" name="PrixUnitaire[]" class="form-control" value="{{ old('PrixUnitaire.' . $index) }}" required oninput="calculateAmount(this)"></td>
                                                <td><input type="number" name="montant[]" class="form-control" value="{{ old('montant.' . $index) }}" readonly></td>
		...
		function addRow() {
			...
			row.innerHTML = `
				<tr class="text-center">
					...
					<td><button type="button" class="btn btn-danger btn-sm  waves-effect waves-light" onclick="removeRow(this)"><i class="ri-delete-bin-6-line"></i></button></td>
				</tr>
			`;
		}
