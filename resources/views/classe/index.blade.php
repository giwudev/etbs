...
@section('content')
	...
	<div class="col-lg-12">
		<div class="card"  id="ticketsList">
			<div class="card-header border-0">
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre}}</h5>
					<div class="flex-shrink-0">
						@if(in_array('add_classe',session('InfosAction')))
							<a class="btn btn-primary btn-label right" href="{{route('classe.create')}}"><i class="ri-add-line label-icon align-middle fs-16 ms-2"></i>Ajouter</a>
						@endif
						@if(in_array('exporter_classe',session('InfosAction')))
			...
			<div class="card-body"><div id='dataRefresh' class="table-responsive table-card mb-4 m-2 giwuRefresh">@include('classe.index-search')</div></div>
		</div><!--end card-->
	</div><!--end col-->
...
@section('JS_content')
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".exporterXls").attr('href','{{url("classe/exporterExcel")}}');
			$(".exporterPdf").attr('href','{{url("classe/exporterPdf")}}');
			// Pagination
			...
			function fetch_page(page){
				...
				$.ajax({ url:"classe?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			$(".exporterXls").attr('href','{{url("classe/exporterExcel")}}?'+filtreData);
			$(".exporterPdf").attr('href','{{url("classe/exporterPdf")}}?'+filtreData);
			...
			return $.ajax({
				url: '{{ url("/classe/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.btn-delete', function () {
			id = $(this).data("id");
			$.ajax({url : '{{ url("classe/AffichePopDelete/") }}/'+id,type : 'GET',dataType : 'html',
