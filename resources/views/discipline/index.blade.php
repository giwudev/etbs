...
@section('content')
	...
	<div class="col-lg-12">
		<div class="card"  id="ticketsList">
			<div class="card-header border-0">
				<div class="d-flex align-items-center"><h5 class="card-title mb-0 flex-grow-1"><i class="{{$icone}} m-2"></i>{{$titre}}</h5>
					<div class="flex-shrink-0">
						@if(in_array('add_discipline',session('InfosAction')))
							<a class="btn btn-primary btn-label right" href="{{route('discipline.create')}}"><i class="ri-add-line label-icon align-middle fs-16 ms-2"></i>Ajouter</a>
						@endif
						@if(in_array('exporter_discipline',session('InfosAction')))
			...
			<div class="card-body"><div id='dataRefresh' class="table-responsive table-card mb-4 m-2 giwuRefresh">@include('discipline.index-search')</div></div>
		</div><!--end card-->
	</div><!--end col-->
...
@section('JS_content')
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(".exporterXls").attr('href','{{url("discipline/exporterExcel")}}');
			$(".exporterPdf").attr('href','{{url("discipline/exporterPdf")}}');
			// Pagination
			...
			function fetch_page(page){
				...
				$.ajax({ url:"discipline?page="+page, data: dr, type: 'GET',
		...
		function funcRecher(){
			...
			$(".exporterXls").attr('href','{{url("discipline/exporterExcel")}}?'+filtreData);
			$(".exporterPdf").attr('href','{{url("discipline/exporterPdf")}}?'+filtreData);
			...
			return $.ajax({
				url: '{{ url("/discipline/")}}',data: filtreData,type: 'GET',
		...
		$(document).on('click', '.btn-delete', function () {
			id = $(this).data("id");
			$.ajax({url : '{{ url("discipline/AffichePopDelete/") }}/'+id,type : 'GET',dataType : 'html',
